# PROMPT MAÎTRE NOVADECORUSA — V2.3

Document de référence consolidé pour la rédaction et la publication d'articles.

**Mise à jour V2.3** : le Cluster 2 (Textures) est désormais complet : 1 Pillar (Post 1351) + 4 Satellites (1385 Layer Textures, 1473 Travertine, 1474 Bouclé, 1475 Raw Wood), tous en brouillon/planifié avec images et métadonnées prêtes. Sur confirmation explicite de Juliana, les placeholders internes ont été convertis en vraies balises `<a href="https://novadecorusa.com/{slug}/">Titre réel de la cible</a>`, exactement selon la procédure V2.0 : relecture live via `context=edit` de chaque post avant conversion, mise à jour ciblée du seul champ `content` par l'API REST, puis resynchronisation du dépôt local. **Anomalie détectée et corrigée** : les Satellites Bouclé (1474) et Raw Wood (1475) portaient chacun un placeholder mal étiqueté pointant vers le Pillar du Cluster 1 (« WarmMinimalism pillar: The Complete Guide to Warm Minimalism ») alors que la phrase environnante et la règle de la section 11 (chaque Satellite doit lier vers le Pillar de **son propre cluster**) désignaient sans ambiguïté le Pillar Textures (Post 1351, « The Best Materials for Warm Minimalism »). Le lien a été corrigé vers le Pillar Textures dans les deux articles ; le lien croisé du Pillar Textures vers le Pillar Cluster 1 (référence de guide complémentaire, hors obligation de la section 11) a lui été conservé tel quel vers le Post 1199. Les changements par rapport à la V2.2 sont signalés par [V2.3].

**Mise à jour V2.2** : confirme définitivement la règle V1.8 après un test délibéré. Un bloc image « hero » (1600x900) prépendu en haut du corps de l'article a été testé isolément sur un seul post (Post ID 1385), avec sauvegarde préalable du contenu original. Après inspection, Juliana a choisi d'annuler l'idée et de revenir au contenu d'origine : **seule l'image mise en avant native de WordPress (« Set featured image », réglée en 1600x900) sert de visuel d'en-tête**, jamais un bloc image supplémentaire prépendu au contenu. Cela reconfirme et clôt la question laissée ouverte par la V1.8. Le post 1385 a été restauré à l'identique depuis sa sauvegarde, et la fonction technique correspondante (`buildHeroImageBlock`) a été retirée du script après le test. Les changements par rapport à la V2.1 sont signalés par [V2.2]. Cette règle est absolue et s'applique automatiquement à tous les articles du calendrier éditorial, sans qu'il soit nécessaire de la redemander.

**Mise à jour V2.1** : lancement du Cluster 2 (Textures) avec son Pillar, « The Best Materials for Warm Minimalism » (Post ID 1351, catégorie Textures). Deux clarifications techniques actées à cette occasion. (1) Quand « le bloc JSON-LD » est demandé au singulier, Article et FAQPage sont combinés dans un seul `<script type="application/ld+json">` via `"@graph": [...]`, plutôt que deux balises `<script>` séparées ; les deux formats restent valides, celui-ci est utilisé par défaut chaque fois qu'un bloc unique est explicitement demandé. (2) Rappel appliqué strictement : un Pillar respecte la fourchette de 2 800 à 3 500 mots de prose narrative (section 8.0) et prévoit un lien (placeholder tant que rien n'est publié) vers chacun des 4 Satellites de **son propre cluster** (section 11), pas seulement vers le Pillar d'un autre cluster déjà traité. Les changements par rapport à la V2.0 sont signalés par [V2.1].

**Mise à jour V2.0** : le Cluster 1 (WarmMinimalism) est désormais complet : 1 Pillar + 4 Satellites, tous en brouillon avec images et métadonnées prêtes. Sur confirmation explicite de Juliana que le cluster est complet, les 4 liens internes placeholders reliant le Pillar à ses 4 Satellites (et les liens croisés pertinents entre Satellites) ont été convertis en vraies balises `<a href="https://novadecorusa.com/{slug}/">Titre réel de la cible</a>`, en anticipation d'une publication groupée prochaine. Cette conversion a été faite par une mise à jour ciblée du seul champ `content` via l'API REST (jamais `title`, `slug`, `categories`, `meta` ou `featured_media`), afin de ne jamais écraser des ajustements manuels déjà faits par Juliana directement dans l'éditeur WordPress. **Constat important** : les slugs réels de deux posts (Pillar et Satellite « Why Cold Minimalism Fails ») avaient déjà été modifiés manuellement (ou par RankMath) directement dans WordPress depuis leur dernière intégration, et ne correspondaient plus aux fichiers locaux du dépôt ni, pour le Pillar, au slug mentionné par Juliana elle-même (`warm-minimalist-home-decor`, qui ne correspond à aucun post existant sur le site). Règle définitive : avant toute conversion de placeholders en liens vivants, ou toute mise à jour d'un post déjà modifié depuis sa dernière intégration, Claude Code doit d'abord relire le contenu et le slug réels via l'API REST (`context=edit`) plutôt que de se fier au dépôt local ou aux notes de Juliana, puis synchroniser le dépôt local une fois l'opération terminée. Les changements par rapport à la V1.9 sont signalés par [V2.0]. Cette règle est absolue.
...
- [V1.6] **Format Gutenberg par bloc, jamais un unique wp:html global** : chaque élément du corps (H2/H3, paragraphe, encart image, table des matières, séparateur) est encapsulé individuellement dans son bloc WordPress standard (`wp:heading`, `wp:paragraph`, `wp:quote`, `wp:list`, `wp:separator`), afin que l'article reste pleinement visible et éditable dans l'éditeur visuel Gutenberg. Envelopper tout l'article dans un seul `<!-- wp:html -->` est interdit : cela vide l'éditeur visuel de tout contenu affiché.
- [V1.6] **Protection du bloc Schema JSON-LD** : le JSON à l'intérieur de chaque `<script type="application/ld+json">` est généré sur une seule ligne (JSON compact, sans retour à la ligne interne). Seules ces balises `<script>` sont regroupées dans un unique bloc Gutenberg Custom HTML (`<!-- wp:html -->` ... `<!-- /wp:html -->`) placé tout à la fin du contenu, isolé du reste de l'article, ce qui empêche `wpautop` d'y injecter des `<br />` parasites sans affecter l'affichage du reste du texte dans l'éditeur
...
- Champs SEO (Meta Title / Meta Description / Focus Keyword) : [V1.7] renseignés via l'endpoint dédié `POST /wp-json/rankmath/v1/updateMeta` (`objectType: "post"`, `objectID`, `meta: {rank_math_title, rank_math_description, rank_math_focus_keyword}`), seul mécanisme confirmé fonctionner avec RankMath ; un essai via l'objet `meta` standard de `/wp/v2/posts` est conservé uniquement en repli pour Yoast, si ce plugin venait à être utilisé à la place
- [V1.7] **Champs jamais touchés, donc toujours préservés** : `featured_media` (image mise en avant) et les réglages de layout Kadence (`_kad_post_layout`, `_kad_post_content_style`, etc.) ne sont jamais inclus dans le payload d'intégration ; un champ absent du payload REST n'est jamais modifié par WordPress, donc toute valeur déjà définie par Juliana dans l'éditeur reste intacte après chaque création ou mise à jour
